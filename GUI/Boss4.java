...
public class Boss4 extends JFrame implements KeyListener{
	...
	private JLabel[] bossAttackL = new JLabel[5];
	private JLabel bossAttackL2;
	
	private int weaponGrade, str, weaponUpGrade, totalDamage, potion1, potion2;
    private int waitTime = 0, bossJewel;
    // 각 객체의 크기를 가져옴
    private Rectangle[] bossAttackR = new Rectangle[5];
    private Rectangle bossAttackL2R, bossAttackR2;
	...
	private Timer bossAttackTimer, bossAttackTimer2, bossAttack2Timer, bossAttack2Timer2, attackTimer, jumpTimer, skillQTimer, skillWTimer, skillETimer, skillRTimer;
	private Timer coolTimeQ, noticeQ, coolTimeW, noticeW, coolTimeE, noticeE, coolTimeR, noticeR;
	private int bossMaxHp=1000, bossHp=1000; // 보스 체력 (DB 사용해야하나 생각중)
	...
	private ImageIcon bossIcon = new ImageIcon("src/img/boss3.gif");
	private ImageIcon bossAttackIcon = new ImageIcon("src/img/boss3attack.png");
	private ImageIcon bossAttackIcon2 = new ImageIcon("src/img/boss3dragon.gif");
	private ImageIcon bossMapIcon = new ImageIcon("src/img/boss3map.png");
	...
	public Boss4(Boss boss) {
		...
		try {
			...
			bossJewel = inventoryDAO.getInventoryBossjewel4(1);
		} 
		...
		for (int i = 0; i < bossAttackL.length; i++) {
			...
			bossAttackL[i].setSize(174, 209);
			bossAttackL[i].setVisible(false);
			bossAttackL[i].setIcon(bossAttackIcon);
			battleP.add(bossAttackL[i]);
		}
		
		bossAttackL2 = new JLabel();
		bossAttackL2.setSize(154,100);
		bossAttackL2.setVisible(false);
		bossAttackL2.setIcon(bossAttackIcon2);
		battleP.add(bossAttackL2);
		...
		bossL.setBounds(380,200,248,245);
		battleP.add(bossL);
    ...
    private void bossAttack() {
		int attack_x[] = new int[5];
    	for(int i=0; i<bossAttackL.length; i++) {
    		int randomx = r.nextInt(0,895);
    		attack_x[i] = randomx;
        	bossAttackL[i].setVisible(true);
        	bossAttackL[i].setLocation(attack_x[i], -130);
    	}
        if (bossAttackTimer != null && bossAttackTimer.isRunning()) {
            bossAttackTimer.stop();
        }
    ...
    private void bossAttackCoolTime() {
   	 	bossAttackTimer2= new Timer(6000, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
            	bossAttack();
            }
	...
	private void bossAttack2() {
		bossAttackL2.setBounds(1140, 390, 154, 100);
		if (bossAttack2Timer != null && bossAttack2Timer.isRunning()) {
			bossAttack2Timer.stop();
		}
		int delay = 5000; // milliseconds
		int targetX = -154;
		int stepX = 1294 / (delay / 100); // Assuming timer tick every 100 ms
		bossAttackL2.setVisible(true);
		bossAttack2Timer = new Timer(100, new ActionListener() {
			int currentX = 1140;
			@Override
			public void actionPerformed(ActionEvent e) {
				if (currentX > targetX) {
					currentX -= stepX;
					bossAttackL2.setLocation(currentX, 390);
					bossAttackL2R = bossAttackL2.getBounds();
					bossAttackR2 = new Rectangle(bossAttackL2R.x, bossAttackL2R.y-20, bossAttackL2R.width, bossAttackL2R.height);
					if (bossAttackR2 != null) {
						if (bossAttackR2.intersects(playerhitR)) {
							playerHp -= 6;
							try {
								playerDAO.updatePlayerHp(1, playerHp);
							} 
					...
					bossAttackL2.setLocation(1140, 390);
					bossAttackL2.setVisible(false);
					bossAttack2Timer.stop();
				}
			}
		});
		bossAttack2Timer.start(); // 타이머 시작
	}
    
    private void bossAttackCoolTime2() {
   	 	bossAttack2Timer2= new Timer(10000, new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
            	bossAttack2();
            }
        });
   	 	bossAttack2Timer2.start(); // 타이머 시작
    }
	...
	private void gameOver() {
		if (playerHp <= 0) {
			...
			if (bossAttack2Timer2 != null && bossAttack2Timer2.isRunning()) {
				bossAttack2Timer2.stop();
			}
			try {
				playerDAO.updatePlayerHp(1, playerHp);
			} catch (SQLException e) {
			...
			try {
				playerDAO.updatePlayerHp(1, playerHp);
				inventoryDAO.updateInventoryBossjewel4(1, bossJewel);
			} catch (SQLException e) {
				e.printStackTrace();
			}
			boss.hpRenewal();
