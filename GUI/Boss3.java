...
public class Boss3 extends JFrame implements KeyListener{
	...
	private JLabel[] bossAttackL = new JLabel[4];
    ...
    // 각 객체의 크기를 가져옴
    private Rectangle[] bossAttackR = new Rectangle[4];
	...
	private int bossMaxHp=700, bossHp=700; // 보스 체력 (DB 사용해야하나 생각중)
	...
	public Boss3(Boss boss) {
		...
		try {
			...
			bossJewel = inventoryDAO.getInventoryBossjewel3(1);
		} 
	...
	private void gameOver() {
		if (playerHp <= 0) {
			...
			try {
				playerDAO.updatePlayerHp(1, playerHp);
				inventoryDAO.updateInventoryBossjewel3(1, bossJewel);
			} catch (SQLException e) {
				e.printStackTrace();
			}
			boss.hpRenewal();
